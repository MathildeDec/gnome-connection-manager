...
def passwordToKey(password):
    ...
    # En Python 3, sha256.update() attend des bytes
    if isinstance(password, str):
        password = password.encode("utf-8")
    sha256.update(password)
    key = []
    for c in sha256.digest():
        key.append(c)
    return key
...
# return 16-byte block from an open binary stream
...
def getBlock(fp):
    ...
    # En Python 3, itérer sur bytes donne directement des int
    block = list(raw)
...
# encrypt - wrapper function to allow encryption of arbitrary length
...
def encrypt(text, password):
    ...
    # En Python 3, text peut être str ou bytes ; on travaille en bytes
    if isinstance(text, str):
        text = text.encode("utf-8")
    # PADDING (calculé sur la longueur en octets, après encodage)
    numpads = 16 - (len(text) % 16)
    text = text + bytes([numpads] * numpads)
    ...
    outfile.close()
    # Retourne une str (comme en Python 2)
    return s.decode("ascii")


# decrypt - wrapper function to allow decryption of arbitrary length
...
def decrypt(text, password):
    ...
    # Accepte str ou bytes en entrée
    if isinstance(text, str):
        text = text.encode("ascii")
    fp = io.BytesIO(base64.b64decode(text))
    outfile = io.BytesIO()
